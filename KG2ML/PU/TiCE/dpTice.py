...
def main():
    ...
    parser.add_argument('--promis', action="store_true",
                        help="Set this option to only use the most promising subset (instead of calculating the maximum lower bound)")
    ...
    parser.add_argument("-i", '--nbIts', default=2, type=int,
                        help="The number of times to repeat the the estimation process. Default 2 (first with c_prior=0.5, then with c_prior=c_estimate)")
...
def tice_c_to_alpha(c, gamma):
    return (1 - gamma) * (1 - c) / gamma / c


def tice_wrapper(data, target, k=10, n_folds=10, delta=.2, maxSplits=500, n_splits=40):
    # data = min_max_scale(data)
    data = normalize(data, axis=0)
    gamma = 1 - (target.sum() / target.shape[0])
    target = bitarray(list(target == 1))
    c = tice(data, target, k, np.random.randint(n_folds, size=len(data)), delta=delta, maxSplits=maxSplits, minT=10, n_splits=n_splits)[0]
    return tice_c_to_alpha(c, gamma)
