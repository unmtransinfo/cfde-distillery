...
def dedpul_wrapper(data, target_pu, n_networks=1):
    """
    data: features
    target_pu: labels
    n_networks: n_networks
    Run DEDPUL algorithm with synthetic data
    """
    res = estimate_poster_cv(data, target_pu, estimator='ntc_methods', alpha=None,
                             ...
                             estimate_preds_cv_options={
                                 ...
                                 'n_early_stop': 20,
                                 'catboost_params': {
                                     'iterations': 250, 'depth': 4,
                                     'task_type': 'GPU', 'devices': '0-8',
                                     'l2_leaf_reg': 10, }
                             },
                             )
    return res
...
def main():
    """
    This code calls DEDPUL PU method to estimate alpha.
    """
    ...
    parser.add_argument("-gene_label_file", help="provide input file containing genes and their labels", required=True, default="../../CFDE_data/KG2ML/gene_label_data.csv")
    ...
    for itr in range(n_iterations):
        ...
        # call DEDPUL to get estimated alpha
        Y = 1 - Y  # in DEDPUL, 0 is positive and 1 is unlabeled
        res = dedpul_wrapper(X, Y)
        est_alpha = 1 - res['dedpul'][0]
        print("DEDPUL alpha: {0}".format(est_alpha))
