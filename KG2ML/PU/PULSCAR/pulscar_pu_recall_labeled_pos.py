...
def ml_performance_after_pul(X, y, genes, performance_metrics, params, alpha, unlab_gene_preds, rseed=0, preds_file=None):
    ...
    # ML model
    X, y, y_orig, genes = shuffle(X, y, y_orig, genes, random_state=rseed)
    r = Counter(y)[0] / Counter(y)[1]
    print("Train and test the model (neg/pos): ", r)
    params['scale_pos_weight'] = r
    params['random_state'] = rseed
    ...
    # performance metrics
    tn, fp, fn, tp = confusion_matrix(y_orig, np.round(preds[:, 1])).ravel()
    recall_val = tp / (tp + fn)
...
def main():
    ...
    In this code, for after metrics, the model is trained with class 1 = labeled positives + imputed positives and class 0 = imputed negatives. But Recall
    is computed using original label i.e. class 1 = labeled positives only.
    """
    # local variables to save values
    n_iterations = 40
    alphas = []
    unlab_gene_preds = {}
    before_metrics = {'Sensitivity': []}
    after_metrics = {'Sensitivity': []}
    ...
    parser.add_argument("-pltfile", help="provide filename to save bar plot", required=True, default="../../CFDE_data/KG2ML/results/temp.png")
    parser.add_argument("-predfile", help="provide filename to save predictions", required=True, default="../../CFDE_data/KG2ML/results/preds.tsv")
